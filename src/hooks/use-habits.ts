...
import type { Habit, HabitCompletion, HabitOnSaveData } from '@/lib/types';
...
export function useHabits() {
  ...
  const addHabit = useCallback((habitData: HabitOnSaveData) => {
    if ('id' in habitData) {
      console.warn("addHabit called with an existing habit. Ignoring.");
      return;
    }
  ...
  const updateHabit = useCallback((habitData: HabitOnSaveData) => {
    if (!('id' in habitData)) {
      console.warn("updateHabit called without a habit ID. Ignoring.");
      return;
    }
